      ...
      mounted(){
        ...
        methods:{
          goBack(){
            ...
            this.$router.push('/administrator/check_attendance/'+grade+'?'+this.eventid)
          },
          showStudent(section){
            const grade = this.$route.path.split('/')[3]
              const strand = this.$route.path.split('/')[4]
              this.$router.push('/administrator/check_attendance/'+grade+'/'+strand+'/'+section.replace(/ /g,'_')+'?'+this.eventid)
          },
            reload(){
              const grade = this.$route.path.split('/')[3]
              const strand = this.$route.path.split('/')[4]
